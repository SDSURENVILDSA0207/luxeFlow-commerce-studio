  ...
  return (
    <div className="pointer-events-none fixed bottom-6 right-4 z-[55] flex flex-col items-end gap-3 sm:right-6">
      {open ? (
        <div
          id="concierge-panel"
          className="pointer-events-auto w-[min(calc(100vw-2rem),20rem)] rounded-2xl border border-border-strong bg-surface p-4 shadow-premium-lg"
          role="dialog"
              ...
              if ("external" in action && action.external) {
                return (
                  <li key={action.label}>
                    <a
                      href={action.href}
                      className="premium-ring block rounded-lg border border-border bg-surface-2 px-3 py-2.5 text-body-sm text-foreground transition-colors hover:border-accent/40 hover:text-accent-soft"
